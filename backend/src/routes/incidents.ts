...
router.post('/', authenticateToken, async (req: Request, res: Response) => {
  try {
    const { title, description, location, severity, type, latitude, longitude } = req.body;

    const result = await query(
      `INSERT INTO incidents (title, description, location, severity, type, latitude, longitude, status, created_by)
       VALUES ($1, $2, $3, $4, $5, $6, $7, 'OPEN', $8)
       RETURNING *`,
      [title, description, location, severity, type, latitude, longitude, req.user?.id]
    );
...
router.put('/:id', authenticateToken, async (req: Request, res: Response) => {
  try {
    ...
    const result = await query(
      `UPDATE incidents SET status = $1, severity = $2, notes = $3, updated_at = NOW()
